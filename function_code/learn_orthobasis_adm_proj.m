function [A,X,loss] = learn_orthobasis_adm_proj( Y_noise, D, A_exist, A_init, MAX_ITER, TOL, tau, DISPLAY,if_earlystop) 
...
while ~done,   
    ...
    %}
    if (if_earlystop&&(min(ones(size(D'*A_new))-abs(D'*A_new))<0.01 || iter >= MAX_ITER))
    	done = true; 
    end 
    A_old = A_new; 
